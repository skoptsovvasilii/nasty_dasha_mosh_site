...
def shef():
    ...
    if request.method == "POST":
        ...
        if action=='X':
            ...
            if x is not None:
                ...
                dt_object = datetime.strptime(dat, "%Y-%m-%d %H:%M:%S").date()
                nw = datetime.now().date()
                if nw == dt_object:cur1.execute("UPDATE eat_check SET answer = 0 WHERE id = ? AND date = ?", (ide, dat))
                else:cur1.execute( f'INSERT INTO eat_check (id, student, sheff,  answer) VALUES ({ide}, {0}, {0}, {0})')
                conn_eat1.commit()
                flag=123
        ...
        for key in request.form.keys():
            ...
            try:
                if request.form.get(key) != '':s += f'{r[key]}-{request.form.get(key)} '
            except:pass
        print(s)
        cur12.execute( f'INSERT INTO eat (eat, check1) VALUES (?, ?)', (s, 0))
        conect2.commit()
    ...
    for i in left:
        ...
        s = s.date()
        if date.today()==s:count_dano -= i[3]




    if flag == 123:return render_template("shef.html", flag=123, mes=mes, cn=count_dano)  # всего одна строка!
    else:return render_template("shef.html", flag=flag, mes=mes, cn=count_dano)
...
def index():
    ...
    if request.method == "POST":
        ...
        if x1 == None:
            if action == "V":
                if x[0]>=1:
                    do_eat_are_uoy="Отлично! Вы сказали, что вы поели!"
                    cur.execute(
                ...
                else: do_eat_are_uoy = "У Вас недостаточно средств"
            ...
            if action=="X":
                if x[0]>=1:
                    ...
                    conn1.commit()
                    do_eat_are_uoy = "Хорошо! Мы учли это!"
                else: do_eat_are_uoy = "У Вас недостаточно средств"

        else:
            if x1 == 1:do_eat_are_uoy = 'Сегодня Вы у нас по ели'
            else:do_eat_are_uoy = "Сегодня Вы у нас не ели"
    ...
    if limit > timedelta(days=0): verd = "Оплачено"
    else: verd = "Не оплачено"
    cur1.execute("SELECT * FROM eat_check WHERE id = ? ORDER BY date ASC", (id_man,))
    results =   cur1.fetchall()
    dt = datetime.now()
    x1 = None
    ...
    if x1!=None:
        if x1 == 1:
            do_eat_are_uoy = 'Сегодня Вы у нас поели'
        else:
            do_eat_are_uoy = "Сегодня Вы у нас не ели"
    ...
    wont_that = {
        ...
        "курица": [
            [ "котлеты куриные", "суп куриный", 'лапша домашняя курица', "суп куриный сухарики"],
            ["гречневая каша"]
        ],
        "мясо": [
            [ "биточки мясные макароны", "телятина картофель",
             "суфле мясо", "котлета рыбная паровая", "говядина в щах", "свинина в гуляше"],
            ["яблоко свежее"]
        ],
        "молоко": [
            [ "сырники сгущёнка", "вареники творог",
             "йогурт", "рис молоко", "кефир", "кукурузные хлопья йогурт", 'творог мёд орехи', 'ватрушка творог'],
            ["винегрет овощной"]
        ],
        "цитрус": [
            [ "сок апельсин", "лимонад апельсин"],
            ["компот яблочный"]
        ],
        "сахар": [
            ["хлебцы джем", "пирожки капуста", "миндаль", "халва подсолнечник"],
    ...
    return render_template("index.html", name=x123[0], pay=f'{verd}{pays}', eat=do_eat_are_uoy, br=br)

allerg = None

options = {"fish": "Рыба","chic": "Курица","meat": "Мясо","milk": "Молоко","citr": "Цитрус","saxa": "Сахар"}
    ...
    if request.method == 'POST':
        ...
        for i in allergy:
            if i in preference:
                ...
                return render_template("check.html", result_text=result_text, allergies_list=allergies_list,preferences_list=preferences_list)
